...
The 5-minute heartbeat invokes this only after health validators pass. Each run
reads RIO's persisted work status, asks the configured AI hierarchy for exactly
one highest-impact safe continuation task, executes through the guarded executor,
and persists the result so the next cycle continues from where this one stopped.
"""
import json
import os
import sys
...
def main():
    ...
    memory_context = {
        ...
        "recent_history": (memory.get("history") or [])[-8:],
    ...
    instruction = (
        ...
        "Do not restart from zero and do not repeat the last completed task unless evidence shows a necessary corrective update. "
        "Choose EXACTLY ONE highest-impact safe next task that materially advances the locked six execution pillars and primary revenue objective. "
    ...
    record(
        "WORKING",
        current_task=memory.get("next_task") or "Selecting highest-impact safe Phase-2 task from persistent memory.",
        engine="selecting",
        validators="PRECHECK_PASS",
    ...
    if plan.get("intent") != "execute":
        ...
        record(
            "VICKY_ACTION_REQUIRED",
            current_task=memory.get("next_task") or "Autonomous continuation",
            engine=engine,
            validators="NOT_RUN",
            result=blocker,
            blocker=blocker,
            founder_action_needed=True,
        )
        notify("⚠️ RIO FOUNDER ACTION REQUIRED\n" + blocker[:3000])
        print("[autonomous_cycle] Founder action required:", blocker)
        return 0
    ...
    if result.get("ok"):
        ...
        if not next_task:
            next_task = "Choose the next highest-impact safe task using objective, completed history, and current business metrics."
        record(
            "COMPLETED",
            current_task=chosen,
            engine=engine,
            ...
            validators="PASS",
            result=summary or "Autonomous task completed and validated.",
            next_task=next_task,
            blocker=None,
            founder_action_needed=False,
        )
    ...
    record(
        ...
        changed_files=[],
        validators="FAIL",
        result=summary or "Autonomous task failed and was not accepted.",
        next_task=memory.get("next_task"),
        blocker=error,
        founder_action_needed=founder_needed,
    )
    if founder_needed:
