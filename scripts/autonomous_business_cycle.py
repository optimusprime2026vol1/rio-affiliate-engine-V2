...
def main():
    control=jload(CONTROL,{'kill_switch':False});memory=jload(WORK,{});snap=jload(SNAPSHOT,{});health=jload(STATUS,{});commercial_policy=jload(COMMERCIAL_POLICY,{})
    ...
    done=completed(memory);rp=recent_pillars(memory);counts={p:rp.count(p) for p in PILLARS};lastp=rp[-1] if rp else None;forced_rotate=len(rp)>=2 and rp[-1] is not None and rp[-1]==rp[-2];revenue_focus=float(snap.get('revenue_inr') or 0)<=0 and int(snap.get('ready_offers') or 0)>0;base_pillars=(1,4,6) if revenue_focus else tuple(PILLARS);eligible=[p for p in base_pillars if not(forced_rotate and p==lastp)];min_count=min(counts[p] for p in eligible);priority=[p for p in eligible if counts[p]==min_count]
    context={'soul_runtime':'VALID_HARD_GATE','commercial_validation_policy':commercial_policy,'status':memory.get('status','IDLE'),'last_completed':memory.get('last_completed'),'last_result':memory.get('last_result'),'next_task':memory.get('next_task'),'changed_files':memory.get('changed_files') or [],'recent_completed_task_keys':done,'recent_pillars':rp,'pillar_counts_recent':counts,'rotation_priority_pillars':priority,'zero_revenue_ready_offer_focus':revenue_focus,'eligible_pillars_this_cycle':list(base_pillars),'forced_rotate_away_from':lastp if forced_rotate else None,'recent_history':(memory.get('history') or [])[-14:],'recent_audit':tail_audit(),'business_snapshot':{k:snap.get(k,0) for k in ['ready_offers','blocked_offers','content_items','revenue_inr','net_profit_inr','instagram_posted']}}
    rules=("AUTONOMOUS PHASE-2 PORTFOLIO CYCLE. Portable Soul HARD execution gate has passed. The Founder-locked commercial policy in context is mandatory: Day 30 judges execution readiness, organic viability uses the 60-90-day window, compliance is hard pass/fail, forecasts include 0.5/1/2/3% sensitivity, resource/failure controls are required, and two failed niche pilots require Victor strategy review before a third. Continue persistent memory and choose EXACTLY ONE safe repository task. Six locked pillars are: "+json.dumps(PILLARS)+". Apply scheduling: external waits are WAITING; no repeated/substantially cloned deliverables; max TWO consecutive tasks per pillar then rotate; prefer least-used pillars; max TWO deliverables per product/campaign before switching until new evidence; PRIMARY AGENDA: generate verified approved revenue as soon as practically possible. Prioritize expected revenue impact x readiness x evidence. While verified revenue is zero and ready_offers > 0, prefer conversion, high-intent content, internal linking, offer-page QA, and Founder-approved publishing preparation using existing repository-verified offers over new affiliate networks, account setup plans, or other Founder-gated expansion. Never describe a merchant, product, commission, availability, or live check as verified unless the repository context contains direct evidence; otherwise label it NOT VERIFIED. Maintain parallel progress; Founder-only gates remain protected. If no safe independent task exists respond WAITING_EXTERNAL:. founder_message must state PILLAR:<1-6>, task, why, changes, NEXT_TASK:. Memory:\n"+json.dumps(context,ensure_ascii=False))
