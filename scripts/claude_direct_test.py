...
if not KEY:
    ...
    payload={
      ...
      'max_tokens':300,
      'temperature':0,
      'system':'You are being tested as the primary reasoning engine for RIO. Reply concisely and exactly as requested.',
      'messages':[{'role':'user','content':'Return valid JSON only: {"status":"PASS","engine":"claude","capability":"reasoning+structured-output","note":"direct API test successful"}'}]
    }
    ...
    try:
        with urllib.request.urlopen(req,timeout=90) as r:
            body=json.load(r)
        text=''.join((p.get('text') or '') for p in body.get('content',[]) if p.get('type')=='text').strip()
        result['http_status']=200
        result['response']=text[:1000]
        try:
            parsed=json.loads(text)
            result['parsed']=parsed
            result['ok']=parsed.get('status')=='PASS' and parsed.get('engine')=='claude'
        except Exception as e:
            result['error']='response was not valid JSON: '+str(e)
    except urllib.error.HTTPError as e:
        result['http_status']=e.code
        result['error']=e.read().decode(errors='replace')[:1200]
    except Exception as e:
        result['error']=str(e)
