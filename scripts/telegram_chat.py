...
ROOT=os.path.join(os.path.dirname(__file__),"..");IST=timezone(timedelta(hours=5,minutes=30));STATE_PATH=os.path.join(ROOT,"data","telegram_chat_state.json");STATUS_PATH=os.path.join(ROOT,"data","status.json");CONTROL_PATH=os.path.join(ROOT,"data","control.json");SOUL_PATH=os.path.join(ROOT,"data","SOUL.md");CORE_PATH=os.path.join(ROOT,"data","RIO_3.0_DEFINITION.md");POLICY_PATH=os.path.join(ROOT,"data","AUTONOMY_POLICY.md");COMMERCIAL_POLICY_PATH=os.path.join(ROOT,"data","COMMERCIAL_VALIDATION_POLICY.json");MAX_HISTORY=12;MAX_REPLY_CHARS=3500
BEDROCK_REGION="us-east-1";BEDROCK_URL=f"https://bedrock-mantle.{BEDROCK_REGION}.api.aws/v1/chat/completions";BEDROCK_PRIMARY="qwen.qwen3-coder-next";BEDROCK_EMERGENCY="zai.glm-4.7-flash";DEEPSEEK_URL="https://api.deepseek.com/chat/completions";DEEPSEEK_MODEL="deepseek-chat"
def clean(v):
...
def system_prompt():
 status=jload(STATUS_PATH,{});control=jload(CONTROL_PATH,{});counts=status.get("counts") or {};soul=read_text(SOUL_PATH);soul_context=soul if soul.strip() else "SOUL compatibility file unavailable. Preserve existing RIO core/policy and do not infer missing Soul rules."
...
PORTABLE SOUL:\n{soul_context}\nRIO PROJECT CORE / OBJECTIVE:\n{read_text(CORE_PATH)}\nAUTONOMY POLICY:\n{read_text(POLICY_PATH)}\nFOUNDER-LOCKED COMMERCIAL VALIDATION POLICY:\n{read_text(COMMERCIAL_POLICY_PATH)}\nLive: kill_switch={control.get('kill_switch')}; maintenance_pause_until={control.get('maintenance_pause_until')}; ready_offers={counts.get('ready_offers')}; content_items={counts.get('content_items')}; validators={status.get('all_validators_pass')}; updated={status.get('updated')}.
Return ONLY valid JSON: {{"intent":"respond|execute","summary":"...","risk":"low|medium|high","operations":[],"founder_message":"..."}}
Allowed operations: write_text, write_json, append_text on allowed data/site/scripts paths; and maintenance_pause with integer minutes 1..240 plus optional reason. Operation schema is strict: write_json MUST contain a non-empty JSON object/list in the value field, for example {{\"op\":\"write_json\",\"path\":\"data/example.json\",\"value\":{{\"status\":\"READY\"}}}}. write_text/append_text MUST contain non-blank text in the content field. Never return null/empty value or content. For Founder requests to pause/break/maintenance-stop autonomous work temporarily, use exactly {{"op":"maintenance_pause","minutes":N,"reason":"..."}}. This pauses business execution only; heartbeat/health monitoring remains alive and work auto-resumes after expiry. Never simulate a pause in founder_message without the operation. Never request shell commands, credentials, payment/legal/account actions, or protected-file changes. Reply in user's language."""
def build_messages(history,user_text):
 out=[m for m in history[-MAX_HISTORY:] if m.get("role") in ("user","assistant") and (m.get("content") or "").strip()];out.append({"role":"user","content":user_text});return out
def call_openai_compatible(url,key,model,messages):
 payload={"model":model,"messages":[{"role":"system","content":system_prompt()}]+messages,"temperature":0.2};req=urllib.request.Request(url,data=json.dumps(payload).encode(),method="POST",headers={"Content-Type":"application/json","Authorization":f"Bearer {key}"})
...
def parse_plan(raw):
 text=raw.strip()
 if text.startswith("```"):
  lines=text.splitlines();lines=lines[1:] if lines and lines[0].startswith("```") else lines;lines=lines[:-1] if lines and lines[-1].strip()=="```" else lines;text="\n".join(lines).strip()
 obj=json.loads(text)
 if not isinstance(obj,dict):raise ValueError("plan is not object")
 return obj
def call_llm(history,user_text):
 messages=build_messages(history,user_text);errors=[];providers=[("bedrock-qwen",BEDROCK_KEY,lambda:call_openai_compatible(BEDROCK_URL,BEDROCK_KEY,BEDROCK_PRIMARY,messages)),("deepseek",DEEPSEEK_KEY,lambda:call_openai_compatible(DEEPSEEK_URL,DEEPSEEK_KEY,DEEPSEEK_MODEL,messages)),("bedrock-glm",BEDROCK_KEY,lambda:call_openai_compatible(BEDROCK_URL,BEDROCK_KEY,BEDROCK_EMERGENCY,messages))]
...
def main():
 ...
 for upd in updates.get("result") or []:
  uid=upd.get("update_id");state["offset"]=max(int(state.get("offset") or 0),int(uid)+1) if uid is not None else state.get("offset",0);msg=upd.get("message") or upd.get("edited_message")
  ...
  if low in {"/start","start"}:reply="RIO online. Bedrock Qwen primary; DeepSeek + GLM fallbacks. Guarded executor ACTIVE.";engine="local"
   ...
   send_message(chat_id,"✅ Command received. RIO is processing this now. Primary AI: Bedrock Qwen3 Coder Next.");plan,engine=call_llm(history,text)
